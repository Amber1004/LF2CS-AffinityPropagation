...
from lf2cs.core.produce_class_extended import ProduceClassExtended
...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        self.produce_class = ProduceClassExtended(self.config, self.data_train)
        self.produce_class.init()
        self.task_train.set_samples_class(self.produce_class.classes)

        # model
        self.norm = Normalize(2)
        self.model = RunnerTool.to_cuda(LF2CSNet(low_dim=self.produce_class.lf2cs_out_dim,
        ...
        self.test_tool_lf2cs = LF2CSTestTool(feature_encoder=None, lf2cs_model=self.model,
                                             ...
                                             num_workers=self.config.num_workers, lf2cs_out_dim=self.produce_class.lf2cs_out_dim,
...
if __name__ == '__main__':
    ...
    if args.dataset == MyDataset.dataset_name_omniglot\
        or args.dataset == MyDataset.dataset_name_imbalancedomniglot:
        from configs_omniglot_extended import Config

        now_config = Config(gpu_id=args.gpuid, dataset_name=args.dataset)
        now_config.print_config(now_config.log_file)
    else:
